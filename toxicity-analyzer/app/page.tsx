...
import ToxicityVisualizations from '@/components/ToxicityVisualizations';
...
export default function Home() {
  ...
  const handleAnalyze = async (channelUrl: string) => {
    ...
    try {
      ...
      // Extract channel ID from the first video's channel_id
      if (data.videos && data.videos.length > 0) {
        setSelectedChannelId(data.videos[0].channel_id);
      }
    } catch (err) {
      ...
      <div className="flex flex-col items-center justify-center min-h-screen pt-32 pb-16 relative overflow-hidden">
        ...
        {selectedChannelId && (
          <div className="w-full max-w-6xl mt-12 px-4">
            <ToxicityVisualizations channelId={selectedChannelId} />
          </div>
        )}
